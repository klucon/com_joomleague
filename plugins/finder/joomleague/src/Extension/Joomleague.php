...
final class Joomleague extends Adapter implements SubscriberInterface
{
	...
	private function queryFor(string $kind): QueryInterface
	{
		...
		switch ($kind) {
			...
			default:
				return $db->getQuery(true)->select($common('project','project.id','project.name','project.alias','project.description',"CONCAT(competition.name, ' ', season.name, ' ', sport_type.name, ' ', COALESCE(project.code, ''))",'project.picture','project.access','project.start_date','NULL'))->from($db->quoteName('#__joomleague_project','project'))->innerJoin($db->quoteName('#__joomleague_competition','competition').' ON competition.id=project.competition_id AND competition.published=1 AND competition.access=project.access')->innerJoin($db->quoteName('#__joomleague_season','season').' ON season.id=project.season_id AND season.published=1 AND season.access=project.access')->innerJoin($db->quoteName('#__joomleague_sport_type','sport_type').' ON sport_type.id=project.sport_type_id AND sport_type.published=1')->where('project.published = 1')->order('project.id ASC');
		}
	}
