...
(async () => {
	...
	try {
		...
		const existing = page.getByRole('link', { name: 'Demo položka programu', exact: true });
		if (await existing.count() === 0) {
			...
			await page.locator('#jform_title').fill('Demo položka programu');
			...
			await picker.locator('a.choose_type[data-request*="programitem"]').click();
			await page.locator('#jform_request_match_id').waitFor();
			await page.locator('#jform_request_match_id').selectOption('1');
			await page.locator('#save-group-children-save button').click();
			await page.waitForLoadState('networkidle');
		}

		await page.goto(`${baseUrl}/administrator/index.php?option=com_menus&view=items&menutype=mainmenu`, { waitUntil: 'networkidle' });
		const menuLink = page.getByRole('link', { name: 'Demo položka programu', exact: true });
		await menuLink.waitFor();
		const editUrl = new URL(await menuLink.getAttribute('href'), baseUrl);
		console.log(`PROGRAMITEM_MENU_ID=${editUrl.searchParams.get('id')}`);
	} finally {
		await browser.close();
	}
})();
