...
final class CompetitionsModel extends ListModel
{
	public function __construct($config = [])
	{
		$config['filter_fields'] ??= ['id', 'a.id', 'name', 'a.name', 'code', 'a.code', 'organisation', 'a.organisation', 'published', 'a.published', 'ordering', 'a.ordering'];
	...
	protected function getListQuery()
	{
		...
		if ($search !== '') {
			if (str_starts_with($search, 'id:')) {
				...
				$search = '%' . $search . '%';
				$query->where('(a.name LIKE :searchName OR a.code LIKE :searchCode OR a.organisation LIKE :searchOrganisation OR a.external_code LIKE :searchExternalCode)')
					->bind(':searchName', $search)
					->bind(':searchCode', $search)
					->bind(':searchOrganisation', $search)
