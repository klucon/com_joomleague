...
final class SeasonsModel extends ListModel
{
	...
	{
		$config['filter_fields'] ??= ['id', 'a.id', 'name', 'a.name', 'published', 'a.published', 'ordering', 'a.ordering'];
	...
	protected function getListQuery(): QueryInterface
	{
		...
		$query = $db->createQuery()->select([
			$db->quoteName('a.id'), $db->quoteName('a.name'), $db->quoteName('a.alias'),
			$db->quoteName('a.published'), $db->quoteName('a.ordering'), $db->quoteName('a.checked_out'),
			$db->quoteName('a.checked_out_time'), $db->quoteName('uc.name', 'editor'),
		])->from($db->quoteName('#__joomleague_season', 'a'))->join(
			'LEFT', $db->quoteName('#__users', 'uc'), $db->quoteName('uc.id') . ' = ' . $db->quoteName('a.checked_out')
		);
