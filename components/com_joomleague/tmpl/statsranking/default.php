...
if ($project) {
	StructuredDataHelper::add($this->getDocument(), [
		...
		'@type' => 'Dataset',
		'@id' => StructuredDataHelper::absoluteUrl(Route::_('index.php?option=com_joomleague&view=statsranking&project_id=' . (int) $project->id, false)) . '#dataset',
		'name' => Text::_('COM_JOOMLEAGUE_SITE_STATS_RANKING') . ' - ' . (string) $project->name,
		'url' => StructuredDataHelper::absoluteUrl(Route::_('index.php?option=com_joomleague&view=statsranking&project_id=' . (int) $project->id, false)),
		'isPartOf' => [
		...
		'mainEntityOfPage' => StructuredDataHelper::collectionPage(
			Text::_('COM_JOOMLEAGUE_SITE_STATS_RANKING'),
			array_map(
				static fn (object $row): array => [
					'@type' => 'Observation',
					'name' => (string) ($row->statistic_name ?? ''),
