...
final class ClubsModel extends ListModel
{
	...
	{
		$config['filter_fields'] ??= ['id', 'a.id', 'name', 'a.name', 'short_name', 'a.short_name', 'country_code', 'a.country_code', 'published', 'a.published', 'ordering', 'a.ordering'];
	...
	protected function getListQuery()
	{
		...
		$teamCount = $database->getQuery(true)
			->select('COUNT(*)')
			->from($database->quoteName('#__joomleague_team', 'team'))
			->where('team.club_id = a.id');
		$query = $database->getQuery(true)
			->select('a.*')
			->select('(' . $teamCount . ') AS team_count')
			->select($database->quoteName('editor.name', 'editor_name'))
			->from($database->quoteName('#__joomleague_club', 'a'))
		...
		if ($search !== '') {
			if (str_starts_with($search, 'id:')) {
				...
				$search = '%' . $search . '%';
				$query->where('(a.name LIKE :searchName OR a.short_name LIKE :searchShortName OR a.external_code LIKE :searchExternalCode)')
