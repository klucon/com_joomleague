...
return static function (DatabaseInterface $db, int $actorId, array $requestedProfileCodes): array {
...
$sportOrder = array_values(array_intersect(array_keys($sportNames), $requestedProfileCodes));
if ($sportOrder === []) throw new RuntimeException('No supported sport profile was selected.');
...
if (count($profileRows) !== count($sportOrder)) throw new RuntimeException('One or more selected active sport profiles are missing.');
...
foreach ($profileRows as $profileIndex => $profileRow) {
	...
	foreach ($matches as $matchIndex => $rawMatch) {
		...
		$participantRows = $db->setQuery(
			$db->getQuery(true)
				->select(['participant.id', 'participant.project_entry_id', 'participant.slot_number', 'entry.entry_kind'])
