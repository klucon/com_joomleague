...
$showHelp = (bool) ($params['show_help'] ?? false);
$linkTeams = (bool) ($params['link_teams'] ?? true);
$colorWin = (string) ($params['color_win'] ?? '');
$colorLoss = (string) ($params['color_loss'] ?? '');
$colorDraw = (string) ($params['color_draw'] ?? '');
 ...
 */
$resultStyle = static function (string $outcome) use ($colorWin, $colorLoss, $colorDraw): string {
	$color = match ($outcome) {
		'win' => $colorWin,
		'loss' => $colorLoss,
		default => $colorDraw,
	...
	return $color !== '' ? ' style="color:' . htmlspecialchars($color, ENT_QUOTES, 'UTF-8') . '"' : '';
};

$renderCell = static function (array $results) use ($resultStyle): string {
	$out = [];
	...
	foreach ($results as $r) {
		...
		if ($r->cancel) {
			...
			if ($r->new_match_id > 0) {
				$cell .= ' <a class="jl-matrix-link" href="' . Route::_('index.php?option=com_joomleague&view=matchreport&id=' . (int) $r->new_match_id) . '" title="' . Text::_('COM_JOOMLEAGUE_SITE_NEW_MATCH') . '">&#8594;</a>';
			}
			$out[] = $cell;
			continue;
		}
		...
		if ($r->decision) {
			...
			$outcome = 'draw';
			if ($v1 !== 'X' && $v2 !== 'X') {
				$outcome = $v1 > $v2 ? 'win' : ($v1 < $v2 ? 'loss' : 'draw');
				$cls = $v1 > $v2 ? 'fw-bold text-success' : ($v1 < $v2 ? 'text-danger' : 'text-secondary');
			}
			$out[] = '<a class="jl-matrix-link ' . $cls . '"' . $resultStyle($outcome) . ' href="' . Route::_('index.php?option=com_joomleague&view=matchreport&id=' . (int) $r->id) . '" title="' . Text::_('COM_JOOMLEAGUE_SITE_DECISION') . '">' . $v1 . ':' . $v2 . '</a>';
			continue;
		}
		...
		if ($r->played) {
			...
			$outcome = $e1 > $e2 ? 'win' : ($e1 < $e2 ? 'loss' : 'draw');
			...
			$out[] = '<a class="jl-matrix-link ' . $cls . '"' . $resultStyle($outcome) . ' href="' . Route::_('index.php?option=com_joomleague&view=matchreport&id=' . (int) $r->id) . '">' . $e1 . ':' . $e2 . $suffix . '</a>';
			continue;
		}
		...
		$out[] = '<a class="jl-matrix-link text-muted" href="' . Route::_('index.php?option=com_joomleague&view=matchreport&id=' . (int) $r->id) . '" title="' . $title . '">&#183;</a>';
	}
...
<div class="com-joomleague-site">
	...
	<?php else : ?>
						...
						<tr>
							...
							<?php foreach ($group->teams as $col) : ?>
								<th scope="col" title="<?php echo $this->escape($col->team_name); ?>">
									<?php if ($linkTeams) : ?><a href="<?php echo $teamLink((int) $col->id); ?>"><?php echo $this->escape($col->team_short_name ?: $col->team_name); ?></a><?php else : ?><?php echo $this->escape($col->team_short_name ?: $col->team_name); ?><?php endif; ?>
								</th>
							<?php endforeach; ?>
						</tr>
					</thead>
		...
		<?php if ($showHelp) : ?><p class="jl-site-muted small mt-2"><?php echo Text::_('COM_JOOMLEAGUE_SITE_MATRIX_HINT'); ?></p><?php endif; ?>
	<?php endif; ?>
</div>
