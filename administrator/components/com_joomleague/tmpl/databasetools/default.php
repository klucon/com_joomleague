...
<form action="index.php?option=com_joomleague&view=databasetools" method="post" name="adminForm" id="adminForm">
	<div class="container-fluid">
		...
		<?php foreach ($this->items as $index => $item) : ?><tr><td class="text-center"><input class="form-check-input" type="checkbox" id="cb<?php echo $index; ?>" name="tables[]" value="<?php echo $this->escape($item['name']); ?>" aria-label="<?php echo Text::sprintf('COM_JOOMLEAGUE_DATABASETOOLS_SELECT_TABLE', $this->escape($item['name'])); ?>"></td><td><code><?php echo $this->escape($item['name']); ?></code></td><td class="text-end"><?php echo $item['rows']; ?></td></tr><?php endforeach; ?>
		</tbody></table></div>
	</div>
