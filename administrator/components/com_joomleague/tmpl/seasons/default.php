		...
		<table class="table table-striped" id="seasonList">
			...
			?>
				<tr>
					...
					<th scope="row">
						<?php if ($checkedOut) echo HTMLHelper::_('jgrid.checkedout', $i, $item->editor_name, $item->checked_out_time, 'seasons.', $canChange); ?>
						<?php if ($canEdit && !$checkedOut) : ?><a href="<?php echo Route::_('index.php?option=com_joomleague&task=season.edit&id=' . (int) $item->id); ?>"><?php echo $this->escape($item->name); ?></a><?php else : ?><?php echo $this->escape($item->name); ?><?php endif; ?>
					</th>
					<td class="text-center"><?php echo (int) $item->project_count; ?></td>
					<td class="text-center"><?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'seasons.', $canChange, 'cb'); ?></td>
